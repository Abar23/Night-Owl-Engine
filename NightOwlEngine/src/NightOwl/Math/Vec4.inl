...
namespace NightOwl
{
	...
	Vec4<T> Vec4<T>::Elerp(const Vec4<T>& leftVector, const Vec4<T>& rightVector, const T t)
	{
		...
		T leftVectorZ = std::max(SMALL_EPSILON, std::abs(leftVector.z)) * std::copysign(static_cast<T>(1), leftVector.z);
		T leftVectorW = std::max(SMALL_EPSILON, std::abs(leftVector.w)) * std::copysign(static_cast<T>(1), leftVector.w);
