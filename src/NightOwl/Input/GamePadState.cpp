...
namespace NightOwl::Input
{
	...
	void GamePadState::NormalizeTriggers()
	{
		...
		leftTriggerAxisValue = Math::MapValueInRange(leftTriggerAxisValue, -1.0f, 1.0f, 0.0f, 1.0f);

		float& rightTriggerAxisValue = axes[static_cast<int>(GamePadAxis::RightTrigger)];
		rightTriggerAxisValue = Math::MapValueInRange(rightTriggerAxisValue, -1.0f, 1.0f, 0.0f, 1.0f);
	}
}
